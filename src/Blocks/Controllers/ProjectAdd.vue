<template>
  ...
  <div class="add-project form-wrapper">
    ...
    <div class="left">
      <div class="input-wrapper">
        <label class="help" for="projectName">
          Название проекта
          <span class="important" title="">(обязательно)</span>
    ...
    <div class="right">
      ...
      <div class="link">
        <div class="input-wrapper">
          ...
          <div class="links-form">
            ...
            <button class="baza-button primary" @click="checkLinks()">
      ...
      <div class="categories-wrapper" ref="projectCategoryErr">
        <label class="help">
          Категории проекта
          <span class="important">(обязательно)</span>
          <svg xmlns="http://www.w3.org/2000/svg" v-tippy="{content: 'Загрузите аватар проекта, размером  110x110 пикселей. Поддерживаемые форматы: jpeg, jpg, png, gif и webp.', theme: 'light'}" width="16" height="16" viewBox="0 0 16 16" fill="none">
      ...
      <div class="exchanger-advanced" v-if="isExchanger === true">
        <div class="input-wrapper">
          <label class="help" for="projectName">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" v-tippy="{content: 'Минимальныо возможная сумма обмена в рублях', theme: 'light'}" width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path d="M7.95281 10.8V8M7.95281 5.2H7.95993M15.0758 8C15.0758 11.866 11.8868 15 7.95281 15C4.01886 15 0.829773 11.866 0.829773 8C0.829773 4.13401 4.01886 1 7.95281 1C11.8868 1 15.0758 4.13401 15.0758 8Z" stroke="#A8A8A8" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </label>
          <div class="input">
  ...
  <div class="links">
    ...
    <div class="link" v-for="link of project.links"
      ...
      <div class="link-name">
        ...
        <a v-tippy="{content: `Ссылка: ${link.link}, нажмите, что бы проверить работоспособность`, theme: 'light'}" target="_blank" :href="link.link">
        {{ link.name }}
        </a>
  ...
  methods: {
    uploadAvatar(e){
      ...
      if (image.type !== "image/jpeg" &&
        ...
        this.project.avatarFilePath = image.name
    ...
    uploadBanner(e){
      ...
      if (image.type !== "image/jpeg" &&
        ...
        this.project.bannerFilePath = image.name
    ...
    checkLinks() {
      ...
      // TODO ПОЛЯ ОБМЕННИКОВ НЕ РАБОТАЮТ!

      // TODO НЕ ВИДНО ЧТО ФАЙЛЫ ЗАГРУЖЕНЫ

    },
    checkForm () {
      if (!this.project.name) {
        this.errors.projectNameErr = 'Название проекта должно быть не менее 1 символов'
        this.$refs.projectName.classList.add('error')
      } else {
        delete this.errors.projectNameErr
        this.$refs.projectName.classList.remove('error')
      }
      ...
      if (this.project.description.length < 30 || this.project.description.length > 65535) {
        ...
        this.$refs.projectDescription.classList.add('error')
      } else {
        delete  this.errors.descriptionErr
        this.$refs.projectDescription.classList.remove('error')
      }

      if (this.project.categoryIds.length === 0) {
        this.errors.projectCategoryErr = 'Не выбрана категория проекта'
        this.$refs.projectCategoryErr.classList.add('error')
      } else {
        delete this.errors.projectCategoryErr
        this.$refs.projectCategoryErr.classList.remove('error')
      }
      ...
      if ( this.project.links.length <= 0) {
        this.errors.projectCategoryErr = 'Не добавлена ни одна ссылка'
        this.$refs.projectCategoryErr.classList.add('error')
      } else {
        delete this.errors.projectCategoryErr
        this.$refs.projectCategoryErr.classList.remove('error')
