<template>
  <div class="project-menu" v-if="canEdit" ref="adminMenuDiv">
    ...
    <div class="project-menu-dropdown" v-if="adminMenu.show">
      <project-menu-controller-items
          :placeholder="placeholder"
          :placeholder-menu="placeholderMenu"
          :advanced-menu="advancedMenu"
          @user-action="emit => callModalWithData(emit.component, emit.data, emit.headline)"
          @close-dropdown="adminMenu.show = false"
          :project="project" />
    </div>
  </div>
...
const callModalWithData = (component, data, headline) => {
  PopupData.data = data
  PopupData.component = component
  PopupData.headline = headline
